...
for i=1:W
    for j=1:H
        for k=1:N
            d=j*h+sqrt(((k-i)*w)^2+(j*h)^2);
            % 计算发射到接收传感器的距离，基于两点距离公式，探头与图像中间对齐（为保证中间较强信号保留）
%             d=sqrt(((i-W/2)*w-(k-N/2)*pitch)^2+(j*h)^2)+j*h;
            ...
            if(time<=size(sensor_data,1))
                RF(i,j) = RF(i,j) + sensor_data(time,k,i).*DOA_window(j,k).*Lateral_window(k).*Axial_window(j);
%                 RF(i,j) = RF(i,j) + sensor_data(time,k,i).*Lateral_window(k);
            end
        end
    end
end
...
log_env=log(log_env+0.1); % 取对数并添加常数以避免出现零值
